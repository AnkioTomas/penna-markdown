...
describe("extends/code_block", () => {
  ...
  it("parses single-quoted and unquoted title", () => {
    const quoted = engine().render("```bash title='run.sh'\necho hi\n```");
    expect(quoted.html).toContain('data-title="run.sh"');
    expect(quoted.html).toContain('class="cherry-code-block__title">run.sh</span>');

    const bare = engine().render("```bash title=Makefile\nall:\n```");
    expect(bare.html).toContain('data-title="Makefile"');
  });

  it("falls back to plain GFM code when extension disabled", () => {
    const { html } = base().render("```js\nconst a = 1;\n```");
