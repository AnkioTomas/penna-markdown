  ...
  section.innerHTML = `
    <div class="case-header">
      <div>
        ...
        <div class="meta">${item.section}</div>
      </div>
...
function runCase(item) {
  ...
  return {
    ...
    skipped: false,
    actualHtml: html,
    message: check.ok ? undefined : "HTML 与 CommonMark 期望不一致",
  };
}
...
function runAll({ renderDom = true } = {}) {
  ...
  summaryTotal.textContent = `CommonMark 共 ${allCases.length} 例`;
...
async function init() {
  statusEl.textContent = "加载 CommonMark 用例…";
  const res = await fetch(new URL("../../test/fixtures/commonmark/cases.json", import.meta.url));
  if (!res.ok) {
    statusEl.textContent = "未找到 cases.json，请先运行: pnpm fetch:commonmark";
    return;
  }
...
window.cherryCommonMarkDemo = { transformer, runAll };
