...
- [扩展语法一览](#扩展语法一览)
- [GFM 标准语法](#gfm-标准语法)
- [Cherry 行内扩展](#cherry-行内扩展)
- [行内语法混搭](#行内语法混搭)
- [Cherry 块级扩展](#cherry-块级扩展)
- [容器与布局](#容器与布局)
- [卡片体系](#卡片体系)
- [代码与图表](#代码与图表)
- [媒体嵌入](#媒体嵌入)
...
- [Cherry 与 GFM 对照表](#cherry-与-gfm-对照表)
- [规模与性能压力段](#规模与性能压力段)
...
| 扩展语法 | `demo/extends` | Cherry 扩展分条演示 |
:::
...
**Cherry Markdown Next** 借鉴 [Cherry Markdown](https://github.com/Tencent/cherry-markdown) 思路，核心是一条清晰的管线：
...
标题内可嵌套行内：**粗体标题**、*斜体*、`code`，以及属性 **重点**{.important}。
...
![行内图片](http://iph.href.lu/320x180?text=Inline&bg=2563eb)

![带 title](http://iph.href.lu/480x270?text=Titled&bg=3498db "图片 title 属性")

![宽图](http://iph.href.lu/640x200?text=Wide&bg=2c3e50) 与 ![竖图](http://iph.href.lu/200x360?text=Tall&bg=16a085) 并排

引用式图片：

![引用图][img-ref]

[img-ref]: http://iph.href.lu/240x160?text=Ref&bg=9b59b6 "引用 title"
...
嵌套：==**加粗高亮**==、==*斜体*==、==`code`==、==[链接](https://example.com)==
...
图片 ![pic](http://iph.href.lu/80x80?text=Pic&bg=6366f1){.rounded}
...
> 发布 **Cherry Markdown Next** [v0.1.0]{.tip .top} ：支持 ==高亮==、!! 剧透 !! {click}、:rocket:、$E=mc^2$、H~2~O、[^ref] 脚注引用、%% 内部备注 %% 以及 [文档]([[repo]]){.important}。
...
- [x] 已完成：渲染 `$x^2$` 与 ~~旧方案~~
- 普通项：**粗体** + *斜体* + `code` + [link](https://example.com)
...
Alert 与容器区别：Alert 用 `>` 引用语法；容器用 `:::` 围栏语法。渲染 CSS 类相似（`cherry-alert`）。
...
::: tabs
@tab 概述
Cherry Markdown Next 基于 **AST** 解析，扩展通过 Registry 注册。
...
- 再次右侧
  time=2025-05-20 placement=right type=danger
:::
...
::: link-card 官方文档 link="https://github.com/Tencent/cherry-markdown" icon="https://github.githubassets.com/favicons/favicon.png"

点击整卡跳转，左侧 icon。
:::

::: link-card 封面图 link="https://example.com" image="http://iph.href.lu/80x80?text=DOC&bg=2563eb"
...
::: image-card image="http://iph.href.lu/640x360?text=Lighthouse&bg=1e3a5f" title="阿尔凡齐纳灯塔，阿尔加维，葡萄牙" href="/" author="Andreas Kunz" date="2024/08/16"
...
::: image-card image="http://iph.href.lu/320x180?text=C&bg=059669" title="图片 C"
:::
...
![1](http://iph.href.lu/320x240?text=1&bg=2563eb)

![2](http://iph.href.lu/320x180?text=2&bg=7c3aed)

![3](http://iph.href.lu/320x300?text=3&bg=059669)

![4](http://iph.href.lu/320x220?text=4&bg=dc2626)

![5](http://iph.href.lu/320x260?text=5&bg=ca8a04)

![6](http://iph.href.lu/320x200?text=6&bg=0891b2)
...
!video[演示视频](https://media.w3.org/2010/05/sintel/trailer.mp4)

!video[带 title](https://media.w3.org/2010/05/sintel/trailer.mp4 "视频说明")

!video[带封面](https://media.w3.org/2010/05/sintel/trailer.mp4){poster=http://iph.href.lu/640x360?text=Poster&bg=0f766e}

### 音频

!audio[背景音乐](http://music.163.com/song/media/outer/url?id=453003622.mp3)

!audio[带封面](http://music.163.com/song/media/outer/url?id=453003622.mp3){poster=http://iph.href.lu/320x180?text=Cover&bg=6366f1}

### iframe

!iframe[示例页面](https://example.com)

!iframe[带 title](https://example.com "iframe 说明")

  !iframe[缩进也可](https://example.com)

!iframe[带 query](https://example.com/path?q=1&x=2)
...
::: timeline placement="between"
- v0.1.0 首次发布
  time=2026-01-15 type=success placement=right
...
::: link-card 查看源码 link="[[repo]]" icon="https://github.githubassets.com/favicons/favicon.png"
...
| 能力 | 说明 |
| --- | --- |
| **代码高亮** | 支持 highlight.js / Shiki，渲染时注入 `cherry-code-block__highlighted` |
| **复制按钮** | `cherry-copy-code-button` + `data-cherry-code` |
| **行号 / 行高亮** | `data-cherry-highlight-lines`、`data-line` |
| **代码折叠** | `data-cherry-collapsed` + 展开按钮 |
| **主题** | `cherryTheme` 深浅色切换，影响 Mermaid/ECharts/公式远程图 |
| **代码折叠面板** | `codeCollapse` 客户端交互 |

样式入口：`cherry-markdown-next/transformer.css`
...
!iframe[data](data:text/html,<script>alert(1)</script>) → 拒绝
...
Write-Host "Building cherry-markdown-next..."
pnpm run build
...
<body>
  <div id="app" class="cherry-markdown"></div>
...
/** @param {import('cherry-markdown-next').TransformerEngine} engine */
...
export function Preview({ md }: { md: string }) {
  ...
  return <div className="cherry-markdown" dangerouslySetInnerHTML={{ __html: html }} />;
}
```

```vue title="CherryPreview.vue"
...
<template>
  <div class="cherry-markdown" v-html="html" />
</template>
```
...
#!/usr/bin/env ruby
require 'cherry_markdown_next'

engine = CherryMarkdownNext::TransformerEngine.new
...
use Cherry\MarkdownNext\TransformerEngine;
...
dependencies {
    implementation("com.cherry:markdown-next:0.1.0")
}
```
...
Cherry 主题模式。`auto` 跟随 `prefers-color-scheme`，影响 Mermaid/ECharts/公式远程图配色。
:::
...
是否在代码块右上角显示复制按钮。依赖 `data-cherry-code` 属性存储原始文本。
:::
...
::: field legacyCherryCompat
@type boolean
@deprecated
[v0.9 移除计划]{.danger}

旧版 Cherry Markdown 非标准语法兼容层。新项目请使用 `extends/` 标准扩展。
:::
::::
...
::: timeline placement="between"
- Sprint 1 · 基础管线
  time=2026-01-01 type=success placement=right
...
- Sprint 3 · 质量
  time=2026-03-01 type=warning placement=right
...
![A](http://iph.href.lu/280x200?text=A&bg=2563eb)

![B](http://iph.href.lu/280x320?text=B&bg=7c3aed)

::: card 说明 C
瀑布流中的卡片，引用脚注[^masonry-note]。
:::

![D](http://iph.href.lu/280x240?text=D&bg=059669)

![E](http://iph.href.lu/280x180?text=E&bg=dc2626)

![F](http://iph.href.lu/280x260?text=F&bg=ca8a04)
...
- 容器没有样式？
  ...
  确认已引入 `cherry-markdown-next/transformer.css` 与 `demo/cherry.css`。
...
English · **中文粗体** · *日本語斜体* · Русский · Ελληνικά · עברית · العربية · ไทย · 한국어
...
| 能力 | GFM / CommonMark | Cherry 扩展 | 备注 |
| --- | --- | --- | --- |
...
| r2c1 | **b** | *i* | `c` | [L](https://e.com) | ==h== | :fire: | $n$ | H~2~O | [tag]{.tip} |
...
Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. **Cherry Markdown Next** 在同一句话里再塞入 ==高亮==、$a^2+b^2=c^2$、!!spoiler!! {click}、:rocket:、[链接](https://example.com){.important}、H~2~O、[^perf-long] 与 %%隐藏备注%%，用于观察单行内多种 token 扫描是否稳定。
...
# 附录 A · Cherry Markdown Next 技术白皮书 [Draft]{.warning .top} {#appendix-whitepaper}
...
Cherry 特有语法 parser 集合，通过 Registry 注册，priority 决定匹配顺序。
...
**Base URL**：`https://api.cherry-markdown-next.example/v1`
...
**描述**：Cherry Markdown Next 内部服务接口第 1 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 2 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 3 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 4 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 5 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 6 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 7 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 8 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 9 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 10 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 11 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 12 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 13 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 14 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 15 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 16 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 17 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 18 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 19 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 20 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 21 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 22 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 23 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 24 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 25 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 26 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 27 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 28 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 29 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 30 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 31 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 32 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 33 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 34 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 35 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 36 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 37 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 38 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 39 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
**描述**：Cherry Markdown Next 内部服务接口第 40 号端点，用于演示超长 API 文档排版。实际项目中此端点可能不存在——此处仅作活文档体积与格式回归用途。
...
::: timeline placement="between"
- v0.0.1-alpha
  time=2025-06-01 type=success placement=left
...
- v0.0.5-beta
  time=2025-09-15 type=tip placement=right
...
- v0.1.0-rc1
  time=2026-01-10 type=important placement=left
...
- v0.1.0
  time=2026-03-01 type=success placement=right
...
- v0.2.0-plan
  time=2026-09-01 type=warning placement=left
...
export default defineConfig({
  optimizeDeps: {
    include: ["cherry-markdown-next/transformer", "cherry-markdown-next/renderer"],
  },
});
```
...
export default async function DocPage({ params }: { params: { slug: string } }) {
  ...
  return <article className="cherry-markdown" dangerouslySetInnerHTML={{ __html: html }} />;
}
```
...
在生产部署第 6 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.7 检查项 7：脚注跳转

在生产部署第 7 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。
...
在生产部署第 9 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.10 检查项 10：sanitize 开启

在生产部署第 10 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。
...
在生产部署第 11 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.12 检查项 12：CSP 配置

在生产部署第 12 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.13 检查项 13：公式 CDN 可用

在生产部署第 13 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.14 检查项 14：高亮主题加载

在生产部署第 14 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.15 检查项 15：复制按钮 a11y

在生产部署第 15 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。
...
在生产部署第 17 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.18 检查项 18：iframe 白名单

在生产部署第 18 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.19 检查项 19：Frontmatter 校验

在生产部署第 19 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.20 检查项 20：sanitize 开启

在生产部署第 20 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。
...
在生产部署第 22 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.23 检查项 23：公式 CDN 可用

在生产部署第 23 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.24 检查项 24：高亮主题加载

在生产部署第 24 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.25 检查项 25：复制按钮 a11y

在生产部署第 25 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。
...
在生产部署第 26 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.27 检查项 27：脚注跳转

在生产部署第 27 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.28 检查项 28：iframe 白名单

在生产部署第 28 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.29 检查项 29：Frontmatter 校验

在生产部署第 29 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。

### F.4.30 检查项 30：sanitize 开启

在生产部署第 30 阶段，务必验证此项。Cherry Markdown Next 的 Renderer 层默认假设运行在浏览器环境；SSR 场景下应跳过 `delegate` 绑定，仅在客户端 hydrate 后挂载交互。对于不可信用户输入，`syntaxOptions.sanitize` **必须**为 `true`，并定期审查 DOMPurify 配置是否覆盖新增 HTML 标签。性能方面，60KB 以上 Markdown 文档 parse 耗时约 1–50ms 量级（视硬件与语法复杂度而定）；若超过 200ms，应排查是否存在未闭合容器导致的大段误吞，或 Mermaid/ECharts 块过多。==关键路径=={.important} 建议对核心文档做 CI 快照测试：`engine.render(engine.parse(md))` 输出 hash 与 baseline 对比。
...
*附录结束 · Cherry Markdown Next 技术白皮书 Draft · 仓库 [[repo]] · :sparkles:*
...
*Cherry Markdown Next · [[description]] · 仓库 [[repo]] · :sparkles:*
