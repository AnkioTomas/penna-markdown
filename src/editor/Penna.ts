...
/**
 * Penna 主控：搭建 UI 骨架，子模块经 {@link EventBus} 事件总线通讯。
 *
 * ```
 * .penna
 * ├── toolbar
...
export class Penna {
  ...
  private readonly pennaEl: HTMLElement;
  ...
  constructor(
    ...
    this.eventBus = new EventBus(options.debug, "[penna]", this.log);
    ...
    this.previewEl = el("div", "penna-preview");
    const previewMountEl = el("div", "penna-render");
    ...
    this.pennaEl.appendChild(this.bodyEl);

    // 遮罩点击关闭侧边栏
    this.sidebarMaskEl.addEventListener("click", () => {
      this.eventBus.emit("penna:sidebar", { show: false });
    });

    if (statusbar) {
      this.statusbarEl = el("div", "penna-statusbar-wrap");
      this.pennaEl.appendChild(this.statusbarEl);
    }

    this.rootEl.appendChild(this.pennaEl);
    ...
    this.dialogHost = new DialogHost(this.pennaEl, this.eventBus);
    ...
    queueMicrotask(() => {
      ...
      this.eventBus.emit("penna:layout", {
        mode: initialLayout,
      } satisfies PennaLayoutPayload);
