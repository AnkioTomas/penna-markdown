...
export function replaceGraph(container: HTMLElement, dark: boolean): void {
  ...
  container.querySelectorAll(".cherry-math-latex[data-latex]").forEach((img) => {
    if (!(img instanceof Image)) return;
    img.src = img.src.replace(dark ? 'color=black' : 'color=white', dark ? 'color=white' : 'color=black');
  });

  container.querySelectorAll(".cherry-mermaid__img[data-mermaid]").forEach((img) => {
    if (!(img instanceof Image)) return;
  ...
  container.querySelectorAll(".cherry-echarts__img[data-echarts]").forEach((img) => {
    if (!(img instanceof Image)) return;
    if (dark) {
